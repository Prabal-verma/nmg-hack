...
## Project Goal
Build a high-performance, deterministic SEO Command Center. A Claude Code plugin that ingests a Screaming Frog SEO export (`internal_all.csv` + issue CSVs), audits it against the rulebook, prioritizes issues, writes fixes, serves a live dashboard at localhost:7700, and outputs `outputs/report.json` + `outputs/report.html`.
...
## Architecture
3-stage pipeline (Ingest, Detect, Fixer).
...
## Conventions
- Always use pandas for detection to ensure 100% accuracy, handle all IO errors gracefully, and prioritize pipeline stability.
- Commit after each working step with a real message.
- Run `python run.py sample-export/` to test end to end.

## Current State
- Pipeline functional.
- Detector implements 14 rules.
- Fixer agent ready with error-handling logic.
